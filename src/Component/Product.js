...
class Product extends Component {
  ...
  onFilterSubmit = async () => {
    ...
    if (this.state.productData && this.state.productData.data) {
      if (this.state.productData.data.length <= 0) {
        this.setState({ openProductList: true });
      }
    }
    if (
          ...
          >
            <div className="productFliters">
              ...
              {this.props.showSubCategoryFilter && this.props.categoryValue ? (
                <div className="filters">
                  <FormControl fullWidth>
                    <InputLabel className="mdc-floating-label" shrink={true}>
                      Sub Categories
                    </InputLabel>
                    <Select
                      name="subCategories"
