...
export default function RecievedOrderCard(props) {
  ...
  for (var i = 0; i < items; i++) {  
    totalPrice = totalPrice + props.value.items[i].price * props.value.items[i].quantity;
  }  
  ...
  return (
    <Paper className={classes.root}>
      ...
      >
        <Typography className={classes.mobilePop}>{props.value.vendor.mobile}</Typography>
      </Popover>
      ...
      >
        ...
        <Box component="div" m={1} className={classes.chipGroup}>
          ...
          <Chip label={props.value.status} className={classes.status} onClick={orderDetails}/>
        </Box>
      </CardContent>
        <Snackbar
        anchorOrigin={{
      ...
      />
        <SimpleDialog selectedValue={selectedValue} type="none" open={openDialog} data={props.value} date={dateTime} onClose={handleCloseDialog} itemsCount={items} totPrice={totalPrice}/>
    </Paper>
  );
}